...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        level=logging.INFO,
        handlers=[logging.FileHandler("publish_wave.log", encoding="utf-8"), logging.StreamHandler()],
    )
    ...
    def cleanup():
        t = loop.create_task(room.disconnect())
        t.add_done_callback(loop.stop())

    asyncio.ensure_future(main(room))
    for signalnum in [signal.SIGINT, signal.SIGTERM]:
        signal.signal(signalnum, cleanup)
